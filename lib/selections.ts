...
export async function upsertSelectionFromLineItem(
  ...
  if (lineItem.selection_id) {
    ...
    if (fetchError || !existingSelection) {
      ...
      // Update existing selection
      // NOTE: Do NOT copy cost_code, room, or description from line items
 ...
 * DEPRECATED: Selections should NEVER overwrite line item fields.
 * Line items are the source of truth for cost_code, room, description, and pricing.
 * This function now does nothing to preserve the API contract.
 */
export async function syncLineItemFromSelection(
  selectionId: string,
  userId: string
): Promise<void> {
  // NO-OP: Selections should never overwrite line item fields.
  // Line items remain the source of truth for:
  // - cost_code
  // - room
  // - description
  // - pricing fields (labor_cost, material_cost, client_price, etc.)
  // 
  // Selections only store:
  // - allowance
  // - subcontractor
  // - product_name (title)
  // - associated line_item_ids (via selection_id foreign key)
  return
}
